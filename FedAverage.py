...
def parse_arguments():
    ...
    parser.add_argument('--physical_bs', type=int, default=3,
                        help='the max_physical_batch_size of Opacus LDP, decrease if cuda out of memory')
...
# train_dataloader包含了每个客户端的train_dataloader,test_dataloaders同理，这里也可能用迪利克雷分布函数去控制每个客户端分配到的样本数量，原论文作者这样分配的好处可能是为了让训练的损失减少更稳定
...
results_df = results_df._append(
    ...
    ignore_index=True)
results_df.to_csv(
    f'log/E{args.E}/{DATA_NAME}_{NUM_CLIENTS}_{NUM_CLASES_PER_CLIENT}_{MODE}_{MODEL}_{target_epsilon}.csv', index=False)
